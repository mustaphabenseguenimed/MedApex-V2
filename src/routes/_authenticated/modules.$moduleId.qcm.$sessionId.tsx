...
import { generateExplanationsForQuestion, getQuestionExplanations } from "@/lib/explanations.functions";
import { RichText } from "@/components/RichText";
import { CaseRunner } from "@/components/session/CaseRunner";
import { createFlashcard, createFlashcardsFromSession, reportQuestion } from "@/lib/flashcards.functions";
...
import { ArrowLeft, CheckCircle2, XCircle, Loader2, Flag, StickyNote, Timer, Sparkles, Layers, AlertTriangle } from "lucide-react";
...
function QcmRunnerGate() {
  ...
  return <ModuleScopeGate moduleId={moduleId} scope="sessions"><QcmRunner /></ModuleScopeGate>;
}
...
type Grade = { verdict: "correct" | "partial" | "incorrect"; score: number; feedback?: string; userAnswer?: string };
type Session = {
  id: string;
  question_ids: string[];
...
function QcmRunner() {
  ...
  useEffect(() => {
    (async () => {
      const { data: s } = await supabase.from("qcm_sessions")
        .select("id,question_ids,answers,grades,score,total,finished_at,mode,time_limit_seconds,started_at,module_id")
      ...
      if (ids.length > 0) {
        ...
        const mains = ((qs as Question[]) ?? []).sort((a, b) => ids.indexOf(a.id) - ids.indexOf(b.id));
        const caseIds = mains.filter(m => m.type === "cas_clinique").map(m => m.id);
        let subs: Question[] = [];
        if (caseIds.length > 0) {
          const { data: sq } = await supabase.from("questions").select("*").in("parent_id", caseIds);
          subs = (sq as Question[]) ?? [];
        }
        ...
        for (const m of mains) {
          ...
          if (m.type === "cas_clinique") {
            ordered.push(...subs.filter(sb => sb.parent_id === m.id).sort((a, b) => a.sort_order - b.sort_order));
          }
        }
        ...
        if (!sess.finished_at) {
          ...
          if (firstOpen > 0) {
            ...
            toast.info(tr("Session reprise à la question {n}/{total}").replace("{n}", String(firstOpen + 1)).replace("{total}", String(ordered.length)));
          }
        }
        ...
        if (u.user && orderedIds.length > 0) {
          const [{ data: fs }, { data: ns }] = await Promise.all([
            supabase.from("question_flags").select("question_id").eq("user_id", u.user.id).in("question_id", orderedIds),
            supabase.from("question_notes").select("question_id,body").eq("user_id", u.user.id).in("question_id", orderedIds),
          ]);
  ...
  type Step = { kind: "single"; q: Question } | { kind: "case"; parent: Question; subs: Question[] };
  const steps = useMemo<Step[]>(() => {
    const out: Step[] = [];
    for (const qq of questions) {
      if (qq.type === "cas_clinique") {
        out.push({ kind: "case", parent: qq, subs: questions.filter((s) => s.parent_id === qq.id) });
  ...
  useEffect(() => {
    ...
    const i = steps.findIndex((s) =>
      s.kind === "single" ? s.q.id === resumeQid : s.parent.id === resumeQid || s.subs.some((x) => x.id === resumeQid),
    );
    if (i > 0) setIdx(i);
    setResumeQid(null);
  }, [resumeQid, steps]);
  ...
  const setChoice = (q: Question, choiceIdx: number) => {
    ...
    if (q.type === "qcm") {
      ...
      next = cur.includes(choiceIdx) ? cur.filter(v => v !== choiceIdx) : [...cur, choiceIdx].sort();
    } else {
      next = choiceIdx;
    }
  ...
  const submitQroc = async (q: Question) => {
    ...
    if (!text) { toast.error(tr("Écrivez votre réponse")); return; }
    setGradingId(q.id);
    try {
      await supabase.from("qcm_sessions").update({ answers: { ...answers, [q.id]: text } }).eq("id", sessionId);
  ...
  const finish = async () => {
    ...
    const { error } = await supabase.from("qcm_sessions")
      .update({ answers, score: currentScore, total, finished_at: new Date().toISOString(), duration_seconds: duration })
      .eq("id", sessionId);
    if (error) { toast.error(error.message); return; }
    // Push SM-2 review for each gradable question
    for (const q of gradableQuestions) {
      let correct = false; let sc = 0;
      if (q.type === "qroc") { const g = grades[q.id]; sc = g?.score ?? 0; correct = (g?.verdict === "correct"); }
      else { sc = gradeChoice(q, answers[q.id]); correct = sc >= 1; }
    ...
    autoCards({ data: { sessionId } })
      .then((r: any) => { if (r?.created > 0) toast.success(`${r.created} ${tr("flashcard")}${r.created > 1 ? "s" : ""} ${tr("ajoutée")}${r.created > 1 ? "s" : ""}`); })
      .catch(() => {});
  };
  ...
  const doFlag = async (q: Question) => {
    try {
      ...
      setFlagged((prev) => {
        ...
        if (res.flagged) next.add(q.id); else next.delete(q.id);
        return next;
      });
  ...
  const saveNoteFor = async (q: Question, body: string) => {
    ...
    try { await saveNote({ data: { questionId: q.id, body } }); }
    catch (e: any) { toast.error(e.message ?? tr("Erreur")); }
  };
  ...
  return (
    <div className="min-h-screen bg-background">
      <header className="border-b"><div className="mx-auto max-w-3xl px-6 py-4 flex items-center justify-between">
        <Button asChild variant="ghost" size="sm"><Link to="/modules/$moduleId/entrainement" params={{ moduleId }}><ArrowLeft className="mr-1.5 h-4 w-4" />{tr("Sessions")}</Link></Button>
        <div className="flex items-center gap-3 text-sm text-muted-foreground">
          {isExam && timeLeft != null && !reviewing && (
            <div className={`flex items-center gap-1.5 font-mono ${timeLeft < 60 ? "text-red-600" : ""}`}>
      ...
      <main className="mx-auto max-w-3xl px-6 py-8 space-y-4">
        {!reviewing && (
          <div className="h-1.5 w-full rounded bg-muted overflow-hidden">
            <div className="h-full bg-primary transition-all" style={{ width: `${((idx + 1) / Math.max(1, steps.length)) * 100}%` }} />
          </div>
        )}

        {reviewing ? (
          <>
            <ReviewList questions={questions} answers={answers} grades={grades} score={currentScore} total={gradableQuestions.length} moduleId={moduleId} />
            <div className="flex justify-center gap-2 pt-4">
              <Button asChild variant="outline"><Link to="/modules/$moduleId/entrainement" params={{ moduleId }}>{tr("Nouvelle session")}</Link></Button>
              <Button asChild variant="ghost"><Link to="/questions/history">{tr("Historique")}</Link></Button>
            </div>
          </>
        ) : step?.kind === "case" ? (
          <>
            ...
            <div className="flex justify-between pt-1">
              <Button variant="ghost" disabled={idx === 0} onClick={() => setIdx(idx - 1)}>{tr("Précédent")}</Button>
              {idx < steps.length - 1 ? (
                <Button variant="outline" onClick={() => setIdx(idx + 1)}>{tr("Suivant")}</Button>
              ) : (
                <Button variant="outline" onClick={finish}>{tr("Terminer")}</Button>
              )}
            </div>
            <StepNavigator steps={steps} idx={idx} answers={answers} flagged={flagged} onGo={setIdx} />
          </>
        ) : q ? (
          <>
            <Card>
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <div className="ml-auto flex items-center gap-1">
                    <Button size="sm" variant="ghost" onClick={() => doFlag(q)} title={tr("Flag")}>
                      <Flag className={`h-4 w-4 ${flagged.has(q.id) ? "fill-current text-primary" : ""}`} />
                    </Button>
                    <Sheet>
                      <SheetTrigger asChild>
                        <Button size="sm" variant="ghost" title={tr("Note")}><StickyNote className={`h-4 w-4 ${notes[q.id] ? "text-primary" : ""}`} /></Button>
                      </SheetTrigger>
                      <SheetContent>
                        <SheetHeader><SheetTitle>{tr("Note personnelle")}</SheetTitle></SheetHeader>
                        <Textarea
                          rows={12}
                          className="mt-4"
                        ...
                        />
                        <p className="text-xs text-muted-foreground mt-2">{tr("Sauvegarde automatique à la sortie du champ.")}</p>
                      </SheetContent>
                    </Sheet>
                  </div>
                </div>
              ...
              <CardContent className="space-y-3">
                {q.type === "cas_clinique" && (
                  <p className="text-sm text-muted-foreground italic">{tr("Lisez le cas puis passez à la suite pour répondre aux questions.")}</p>
                )}
                {(q.type === "qcm" || q.type === "qcs") && q.choices && (
                  <div className="space-y-2">
                    {q.choices.map((ch, j) => {
                      ...
                      return (
                        <button key={j} onClick={() => setChoice(q, j)} disabled={showFeedback}
                          className={`flex w-full items-center gap-2 rounded-md border px-3 py-2 text-left text-sm transition ${cls}`}>
                          {showFeedback && isTarget && <CheckCircle2 className="h-4 w-4 text-green-600" />}
                          ...
                          {missed && <span className="text-[10px] text-green-700">{tr("à cocher")}</span>}
                        </button>
                      );
                    })}
                  </div>
                ...
                {q.type === "qroc" && (
                  <div className="space-y-2">
                    <Textarea rows={5} value={answers[q.id] ?? ""} onChange={(e) => setQrocText(q, e.target.value)} placeholder={tr("Votre réponse…")} disabled={!isExam && !!grades[q.id]} />
                    {!isExam && grades[q.id] ? (
                      <QrocFeedback grade={grades[q.id]} modelAnswer={q.model_answer} />
                    ) : !isExam ? (
                      <Button size="sm" variant="secondary" onClick={() => submitQroc(q)} disabled={gradingId === q.id}>
                ...
                {!isExam && revealed.has(q.id) && (q.type === "qcm" || q.type === "qcs" || q.type === "qroc") && (q.explanation || q.model_answer) && (
                  <div className="rounded-md border border-primary/30 bg-primary/5 p-3 space-y-2">
                    <div className="text-xs font-semibold text-primary flex items-center gap-1"><Sparkles className="h-3 w-3" />{tr("Correction")}</div>
                    {q.type === "qroc" && q.model_answer && (
                      <div className="text-xs"><span className="font-medium">{tr("Réponse attendue : ")}</span><span className="whitespace-pre-wrap">{q.model_answer}</span></div>
                    )}
                    {q.explanation && <RichText autoColor html={q.explanation} className="text-sm prose prose-sm dark:prose-invert max-w-none" />}
                  </div>
                )}
                <div className="flex justify-between pt-3">
                  ...
                  <div className="flex gap-2">
                    {!isExam && !revealed.has(q.id) && (q.type === "qcm" || q.type === "qcs") && (
                      <Button variant="secondary" disabled={answers[q.id] == null || (Array.isArray(answers[q.id]) && answers[q.id].length === 0)}
                        onClick={() => setRevealed((prev) => new Set(prev).add(q.id))}>{tr("Valider")}</Button>
                    )}
                    {idx < steps.length - 1 ? (
...
type AnyStep = { kind: "single"; q: Question } | { kind: "case"; parent: Question; subs: Question[] };
  ...
  return (
    <div className="flex flex-wrap gap-1">
      {steps.map((s, i) => {
        ...
        const answered = ids.length > 0 && ids.every((id) => answers[id] != null && answers[id] !== "");
...
function QrocFeedbackInner({ grade, modelAnswer }: { grade: Grade; modelAnswer: string | null }) {
  const { tr } = useI18n();
  const color = grade.verdict === "correct" ? "text-green-600" : grade.verdict === "partial" ? "text-amber-600" : "text-red-600";
  return (
    <div className="rounded-md border p-3 text-sm space-y-2 bg-muted/30">
      <div className={`font-semibold ${color}`}>{tr("Score : ")}{Math.round(grade.score * 100)}% — {grade.verdict}</div>
      {grade.feedback && <p className="text-muted-foreground">{grade.feedback}</p>}
      {modelAnswer && <div><div className="text-xs font-medium mb-1">{tr("Réponse attendue :")}</div><div className="text-xs whitespace-pre-wrap">{modelAnswer}</div></div>}
    </div>
  );
}

function ReviewList({ questions, answers, grades, score, total, moduleId }: { questions: Question[]; answers: Record<string, any>; grades: Record<string, Grade>; score: number; total: number; moduleId: string }) {
  const { tr } = useI18n();
  return (
    <>
      <Card><CardContent className="py-6 text-center">
        <p className="text-3xl font-semibold">{Math.round(score * 10) / 10} / {total}</p>
        <p className="text-sm text-muted-foreground mt-1">{total > 0 ? Math.round((score / total) * 100) : 0}%</p>
      </CardContent></Card>
      {questions.map((q, i) => {
        ...
        if (q.type === "cas_clinique") {
          ...
          return (
            <div key={q.id} className="space-y-2 rounded-2xl border border-primary/40 bg-muted/30 p-3">
              <div className="text-xs font-bold uppercase tracking-widest text-primary">{tr("Cas clinique")}</div>
              <RichText html={q.stem} className="prose prose-sm max-w-none whitespace-pre-wrap text-sm dark:prose-invert" />
              <div className="space-y-2">
                {subs.map((s, j) => (
                  <ReviewCard key={s.id} q={s} idx={j} userAns={answers[s.id]} grade={grades[s.id]} moduleId={moduleId} />
                ))}
              </div>
            </div>
          );
        ...
        return (
          <ReviewCard key={q.id} q={q} idx={i} userAns={answers[q.id]} grade={grades[q.id]} moduleId={moduleId} />
        );
      })}
    </>
  );
...
function ReviewCard({ q, idx, userAns, grade, moduleId }: { q: Question; idx: number; userAns: any; grade?: Grade; moduleId: string }) {
  const { tr } = useI18n();
  const target = targetIndices(q);
  ...
  const [reportReason, setReportReason] = useState<"error" | "typo" | "ambiguous" | "outdated" | "other">("error");
  ...
  useEffect(() => {
    (async () => {
      ...
      for (const r of (res as any).rows) map[r.choice_index == null ? "overall" : String(r.choice_index)] = r.body;
      setExpl(map);
    })();
  }, [q.id, getExpl]);
  ...
  const generate = async () => {
    ...
    try {
      ...
      for (const r of (res as any).rows) map[r.choice_index == null ? "overall" : String(r.choice_index)] = r.body;
      setExpl(map);
    } catch (e: any) { toast.error(e.message ?? tr("Erreur IA")); }
    finally { setLoadingExpl(false); }
  };

  const saveCard = async (front: string, back: string, choiceIdx?: number) => {
    try {
      await addCard({ data: { front, back, question_id: q.id, choice_index: choiceIdx ?? null, module_id: moduleId } });
  ...
  return (
    <Card>
      <CardHeader>
        <div className="flex items-center gap-2 flex-wrap">
          <Badge variant="outline">{q.type.toUpperCase()}</Badge>
          {(q.type === "qcm" || q.type === "qcs") && q.polarity === "incorrect" && <Badge variant="destructive">{tr("mauvaises réponses")}</Badge>}
          <div className="ml-auto flex gap-1">
            <Button size="sm" variant="ghost" onClick={generate} disabled={loadingExpl}>
              {loadingExpl ? <Loader2 className="h-4 w-4 animate-spin" /> : <Sparkles className="h-4 w-4" />}
              <span className="ml-1 hidden sm:inline">{tr("Expliquer")}</span>
            </Button>
            <Dialog>
              <DialogTrigger asChild><Button size="sm" variant="ghost"><AlertTriangle className="h-4 w-4" /></Button></DialogTrigger>
              <DialogContent>
                <DialogHeader><DialogTitle>{tr("Signaler la question")}</DialogTitle></DialogHeader>
                <div className="space-y-3">
                  ...
                  <Textarea rows={4} placeholder={tr("Détails (optionnel)")} value={reportDetails} onChange={(e) => setReportDetails(e.target.value)} />
                </div>
                <DialogFooter><Button onClick={submitReport}>{tr("Envoyer")}</Button></DialogFooter>
              </DialogContent>
            </Dialog>
          </div>
        </div>
        <CardTitle className="text-base whitespace-pre-wrap">{idx + 1}. {q.stem}</CardTitle>
      </CardHeader>
      <CardContent className="space-y-2">
        {(q.type === "qcm" || q.type === "qcs") && q.choices && q.choices.map((ch, j) => {
          const isTarget = target.includes(j);
          ...
          return (
            <div key={j} className={`rounded-md border px-3 py-2 text-sm ${isTarget ? "bg-green-500/10 border-green-500" : ""} ${wrong ? "bg-red-500/10 border-red-500" : ""}`}>
              <div className="flex items-center gap-2">
                {isTarget && <CheckCircle2 className="h-4 w-4 text-green-600" />}
                {wrong && <XCircle className="h-4 w-4 text-red-600" />}
                <span className="font-mono text-xs">{String.fromCharCode(65 + j)}.</span>
                <span className="flex-1">{ch}</span>
                <Button size="sm" variant="ghost" title={tr("Ajouter aux flashcards")}
                  onClick={() => saveCard(`${q.stem}\n\nOption ${String.fromCharCode(65 + j)}: ${ch}`, explBody || (isTarget ? tr("✓ Bonne réponse") : tr("✗ Mauvaise réponse")) + (q.explanation ? "\n" + q.explanation : ""), j)}>
                  <Layers className="h-4 w-4" />
                </Button>
              </div>
              {(explBody && (wrong || missed || isTarget)) && (
                <div className="mt-2 text-xs text-muted-foreground whitespace-pre-wrap border-l-2 border-primary/30 pl-2">{explBody}</div>
              )}
            </div>
          );
        })}
        ...
        {expl["overall"] && (
          <div className="rounded-md border border-primary/30 bg-primary/5 p-3 text-sm whitespace-pre-wrap">
            <div className="text-xs font-semibold text-primary mb-1 flex items-center gap-1"><Sparkles className="h-3 w-3" />{tr("Explication IA")}</div>
            {expl["overall"]}
          </div>
        )}
        {q.explanation && <RichText autoColor html={q.explanation} className="text-xs text-muted-foreground pt-2 prose prose-sm dark:prose-invert max-w-none" />}
      </CardContent>
    </Card>
  );
}