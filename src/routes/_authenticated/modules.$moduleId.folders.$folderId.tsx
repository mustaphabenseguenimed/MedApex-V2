...
import { ArrowLeft, FileText, FileType, ListChecks, BookOpen, Link as LinkIcon } from "lucide-react";
...
function FolderGate() {
  ...
  return <ModuleScopeGate moduleId={moduleId} scope="lessons"><FolderView /></ModuleScopeGate>;
}

type Kind = "html" | "pdf" | "richtext" | "quiz" | "link";
type Content = { id: string; kind: Kind; title: string; description: string | null; folder_id: string | null };
...
function FolderView() {
  ...
  useEffect(() => {
    (async () => {
      if (!isUnfiled) {
        const { data } = await supabase.from("module_folders").select("name").eq("id", folderId).maybeSingle();
        if (data) setName(data.name);
      }
      let q = supabase.from("module_contents").select("id,kind,title,description,folder_id").eq("module_id", moduleId).order("sort_order").order("created_at");
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-5xl px-6 py-10">
        ...
        {items.length === 0 ? (
          <Card><CardContent className="py-12 text-center text-muted-foreground">{t("no_content_folder")}</CardContent></Card>
        ) : (
