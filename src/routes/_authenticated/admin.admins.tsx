...
const ALL_PERMS: { key: AdminPermission; label: string; hint: string }[] = [
  ...
  { key: "manage_access", label: "Accès", hint: "Accorder / retirer manuellement les accès" },
  {
    key: "manage_payments",
    label: "Paiements",
    hint: "Valider / rejeter les demandes de paiement",
  },
];
...
function PermissionsPicker({
  ...
  const add = (k: string) => {
    if (!k) return;
    onChange([...value, k as AdminPermission]);
  };
  return (
    <div className="space-y-2">
      <div className="flex flex-wrap gap-1.5 min-h-[32px]">
        {value.length === 0 && (
          <span className="text-xs text-muted-foreground">{tr("Aucune permission")}</span>
        )}
        {value.map((k) => {
        ...
        <div className="flex items-center gap-2">
          <Select onValueChange={(v) => add(v)} value="">
            <SelectTrigger className="h-8 w-64">
              <SelectValue placeholder={tr("Ajouter une permission…")} />
            </SelectTrigger>
            <SelectContent>
              {available.map((p) => (
                <SelectItem key={p.key} value={p.key}>
                  <span className="font-medium">{tr(p.label)}</span>{" "}
                  <span className="text-xs text-muted-foreground">— {tr(p.hint)}</span>
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-6">
        <Card className="max-w-md w-full">
          <CardHeader>
            <CardTitle>{tr("Réservé au super admin")}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            <p className="text-sm text-muted-foreground">
              {tr("Seul le super administrateur peut promouvoir d'autres admins.")}
            </p>
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-4xl px-6 py-8 space-y-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base">{tr("Admins existants")}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            {rows.length === 0 && (
              <p className="text-sm text-muted-foreground">{tr("Aucun admin.")}</p>
            )}
            {rows.map((r) => (
      ...
      <CardContent className="space-y-4">
        <div className="space-y-1">
          ...
          <Input
            ...
            placeholder="ex: collab@exemple.com"
          />
        </div>
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            Utilisez le sélecteur pour ajouter, cliquez sur × pour retirer.
          </p>
        </div>
...
function AdminRowEditor({ row, onChanged }: { row: AdminRow; onChanged: () => void }) {
  ...
  useEffect(() => {
    setSelected(row.permissions as AdminPermission[]);
  }, [row.user_id, row.permissions.join(",")]);

  const save = async () => {
    if (!row.email) {
      toast.error(tr("Email inconnu"));
      return;
    }
    setBusy(true);
    ...
    if (error) toast.error(error.message);
      ...
      toast.success(tr("Permissions mises à jour"));
      onChanged();
    }
  };
  ...
  const revoke = async () => {
    if (
      !(await confirm(`${tr("Retirer les droits admin de")} ${row.email} ?`, {
        variant: "destructive",
      }))
    )
      return;
    ...
    if (error) toast.error(error.message);
      ...
      toast.success(tr("Admin révoqué"));
      onChanged();
    }
  };
  ...
  return (
    <div className="rounded-md border p-3 space-y-3">
      ...
      {row.is_super ? (
        <p className="text-xs text-muted-foreground">
          {tr("Accès total. Les permissions ne s'appliquent pas.")}
        </p>
      ) : (
        <>
          <PermissionsPicker value={selected} onChange={setSelected} />
