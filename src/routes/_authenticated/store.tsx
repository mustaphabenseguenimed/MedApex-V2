...
import { ArrowLeft, Check, ShoppingBag, Sparkles, Copy, Clock, XCircle, CheckCircle2, ExternalLink } from "lucide-react";
import { toast } from "sonner";
import { YEAR_DEADLINE, BUNDLE_DEADLINE, isSalesClosed, formatDeadline } from "@/lib/deadlines";
import { useI18n } from "@/lib/i18n";
import { scopeLabel, type AccessScope } from "@/lib/scopes";
...
function fmt(n: number, currency: string, locale: string) { return new Intl.NumberFormat(locale).format(n) + " " + currency; }
...
function StorePage() {
  ...
  const load = async () => {
    ...
    if (u.user) {
      setUserId(u.user.id);
      const { data: e } = await supabase.from("user_entitlements").select("year, is_bundle, scope").eq("user_id", u.user.id);
      setEnts((e as Entitlement[]) ?? []);
      const { data: r } = await supabase.from("payment_requests")
        .select("id, is_bundle, year, scope, amount_dzd, status, created_at, reject_reason, transaction_ref, proof_path, method_kind")
  ...
  const covers = (entScope: AccessScope, wanted: AccessScope) =>
    entScope === "both" || entScope === wanted;
  const owns = (isBundle: boolean, y: number | null, scope: AccessScope) =>
    ents.some((e) => (e.is_bundle || (!isBundle && e.year === y)) && covers(e.scope, scope));
  const pendingFor = (isBundle: boolean, y: number | null, scope: AccessScope) =>
    requests.find((r) => r.status === "pending" && r.is_bundle === isBundle && r.year === y && r.scope === scope);
  ...
  return (
    <div className="min-h-screen bg-background">
      <header className="border-b"><div className="mx-auto max-w-6xl px-6 py-4 flex items-center justify-between">
        ...
        <h1 className="text-lg font-semibold flex items-center gap-2"><ShoppingBag className="h-5 w-5" />{t("store_title")}</h1>
      </div></header>
      ...
      <main className="mx-auto max-w-6xl px-6 py-10 space-y-10">
          ...
          <Card className="border-primary/40 bg-gradient-to-br from-primary/5 to-transparent">
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-xl">
                <Sparkles className="h-5 w-5 text-primary" />{t("bundle_title")}
              </CardTitle>
            ...
            <CardContent className="space-y-3">
              ...
              <p className="text-xs text-muted-foreground">{t("offer_valid_until")} {formatDeadline(BUNDLE_DEADLINE)}.</p>
            </CardContent>
          </Card>
        )}
        ...
        <div>
          <div className="mb-3 flex items-baseline justify-between gap-3 flex-wrap">
            ...
            <p className="text-xs text-muted-foreground">{t("offers_valid_until")} {formatDeadline(YEAR_DEADLINE)}.</p>
          </div>
                ...
                <Card key={p.year}>
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      <CardTitle className="text-base">{t("year")} {p.year}</CardTitle>
                      {owns(false, p.year, "both") && <Badge variant="secondary"><Check className="h-3 w-3 mr-1" />{t("possessed")}</Badge>}
                    </div>
                  </CardHeader>
                ...
                <div key={r.id} className="rounded-lg border px-4 py-3 text-sm">
                  <div className="flex items-start justify-between gap-3 flex-wrap">
                    <div className="min-w-0">
                      <div className="font-medium">{r.is_bundle ? t("bundle_short") : `${t("year")} ${r.year}`} — {fmt(r.amount_dzd, currency, locale)}</div>
                      <div className="text-xs text-muted-foreground">{new Date(r.created_at).toLocaleString(locale)}</div>
                      <div className="mt-1 text-xs text-muted-foreground space-y-0.5">
                        {r.method_kind && <div>{t("method")} : <span className="text-foreground">{r.method_kind}</span></div>}
                        {r.transaction_ref && <div>{t("reference")} : <code className="bg-muted px-1.5 py-0.5 rounded">{r.transaction_ref}</code></div>}
                        {r.proof_path && (
                          <button onClick={() => openProof(r.proof_path!)} className="inline-flex items-center gap-1 text-primary hover:underline">
                            <ExternalLink className="h-3 w-3" />{t("view_proof")}
                          </button>
                        )}
                      </div>
                      {r.status === "rejected" && r.reject_reason && (
                        <div className="text-xs text-destructive mt-1">{t("reason")} : {r.reject_reason}</div>
                      )}
                    </div>
                    <div>
                      {r.status === "pending" && <Badge variant="outline"><Clock className="h-3 w-3 mr-1" />{t("pending")}</Badge>}
                      {r.status === "approved" && <Badge className="bg-emerald-600 hover:bg-emerald-600"><CheckCircle2 className="h-3 w-3 mr-1" />{t("approved")}</Badge>}
                      {r.status === "rejected" && <Badge variant="destructive"><XCircle className="h-3 w-3 mr-1" />{t("rejected")}</Badge>}
                    </div>
                  </div>
                </div>
              ))}
      ...
      <PaymentDialog target={buyTarget} onClose={() => setBuyTarget(null)} methods={methods} currency={currency} locale={locale} userId={userId} onSubmitted={load} />
    </div>
  );
}

function PaymentDialog({ target, onClose, methods, currency, locale, userId, onSubmitted }: {
  target: Target | null; onClose: () => void; methods: PaymentMethod[]; currency: string; locale: string; userId: string | null; onSubmitted: () => void;
}) {
  const { t } = useI18n();
  ...
  useEffect(() => {
    if (!target) { setTxRef(""); setNote(""); setFile(null); setMethodKind(""); }
  }, [target]);
  ...
  const submit = async () => {
    if (!userId) { toast.error(t("not_authenticated")); return; }
    if (!methodKind) { toast.error(t("choose_method")); return; }
    if (!txRef.trim() && !file) { toast.error(t("provide_ref_or_screenshot")); return; }
    setSubmitting(true);
    try {
  ...
  return (
    <Dialog open={!!target} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{t("pay")} : {target.label}</DialogTitle>
          <DialogDescription>
            {t("amount_to_pay")} : <span className="font-semibold text-foreground">{new Intl.NumberFormat(locale).format(target.amount) + " " + currency}</span>
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          <div>
            ...
            <div className="grid gap-2">
              {methods.length === 0 && <p className="text-sm text-muted-foreground">{t("no_methods")}</p>}
              {methods.map((m) => (
                <label key={m.id} className={`rounded-lg border p-3 cursor-pointer text-sm ${methodKind === m.kind ? "border-primary bg-accent" : ""}`}>
                  <div className="flex items-start gap-2">
                    <input type="radio" name="pm" className="mt-1" checked={methodKind === m.kind} onChange={() => setMethodKind(m.kind)} />
                    <div className="flex-1 space-y-1">
                      <div className="font-medium">{m.label}</div>
                      <div className="text-xs text-muted-foreground">{t("beneficiary")} : <span className="text-foreground">{m.account_holder}</span></div>
                      <div className="flex items-center gap-2">
                        <code className="text-xs bg-muted px-2 py-1 rounded flex-1 break-all">{m.account_number}</code>
                        <Button type="button" size="sm" variant="ghost" onClick={(e) => { e.preventDefault(); copy(m.account_number); }}>
          ...
          <div className="space-y-2">
            ...
            <Input placeholder={t("ref_placeholder")} value={txRef} onChange={(e) => setTxRef(e.target.value)} />
          </div>
          ...
          <div className="space-y-2">
            ...
            <Textarea rows={2} value={note} onChange={(e) => setNote(e.target.value)} placeholder={t("note_placeholder")} />
          </div>
        </div>
