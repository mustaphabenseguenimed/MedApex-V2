...
export const Route = createFileRoute("/_authenticated/questions/new")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Composez une session de QCM à partir de vos modules, cours et filtres.",
      },
...
function NewSessionPage() {
  ...
  const [counts, setCounts] = useState<{ total: number; byType: Record<string, number> } | null>(
    null,
  );
  ...
  useEffect(() => {
    (async () => {
      const [{ data }, all] = await Promise.all([
        ...
        fetchAllRows<{ module_id: string }>(() =>
          supabase.from("questions").select("module_id").is("parent_id", null),
        ).catch(() => []),
      ]);
      setMods((data as Mod[]) ?? []);
      const map: Record<string, number> = {};
      for (const r of (all as { module_id: string }[]) ?? [])
    ...
    (async () => {
      ...
      const [{ data: f }, q] = await Promise.all([
        supabase
          ...
          .in("module_id", selMods)
          .order("name"),
        fetchAllRows<QMeta>(() =>
          supabase
            .from("questions")
            .select("module_id,exam_year,exam_source")
            .in("module_id", selMods)
            .is("parent_id", null),
        ).catch(() => [] as QMeta[]),
      ]);
      ...
      setMeta(q ?? []);
      setSelFolders((prev) =>
        prev.filter((id) => ((f as Folder[]) ?? []).some((x) => x.id === id)),
      );
    })();
  }, [selMods]);
  ...
  useEffect(() => {
    if (!limitTouched) setLimit(total);
    else setLimit((l) => (total > 0 ? Math.min(l, total) : l));
  }, [total, limitTouched]);

  const byYear = useMemo(() => {
    const q = modQuery.trim().toLowerCase();
    const list = q ? mods.filter((m) => m.title.toLowerCase().includes(q)) : mods;
    return list.reduce<Record<number, Mod[]>>((acc, m) => {
      (acc[m.year] ||= []).push(m);
      return acc;
    }, {});
  }, [mods, modQuery]);
  ...
  const autoTitleValue = useMemo(() => {
    ...
    const base =
      selMods.length > 1
        ? `${first?.title ?? tr("Session")} +${selMods.length - 1}`
        : (first?.title ?? tr("Session"));
    return `${base} ${new Date().toLocaleString()}`;
  }, [mods, selMods]);

  const create = async () => {
    if (selMods.length === 0) {
      toast.error(tr("Sélectionnez au moins un module"));
      return;
    }
    setBusy(true);
    try {
      const res: any = await buildFn({
        data: {
      ...
      navigate({
        ...
        params: { moduleId: res.moduleId, sessionId: res.sessionId },
      });
    } catch (e: any) {
  ...
  return (
    <div className="min-h-screen bg-background">
      <SessionTopBar
        ...
        right={
          <SessionPill pulse>
            {poolTotal.toLocaleString("fr-FR")} {tr("questions")}
          </SessionPill>
        }
      />
        ...
        <SessionSection
          ...
          actions={
            <button
              ...
              onClick={() =>
                setSelYears(selYears.length === studyYears.length ? [] : studyYears.map(([y]) => y))
              }
            >
              {selYears.length === studyYears.length && studyYears.length > 0
                ? tr("Tout désélectionner")
                : tr("Toutes les années")}
            </button>
          }
        >
            ...
            {studyYears.map(([y, count]) => {
              ...
              return (
                <button
                  ...
                  type="button"
                  onClick={() =>
                    setSelYears(active ? selYears.filter((v) => v !== y) : [...selYears, y])
                  }
                ...
                >
                  ...
                  <span className="ml-2 text-xs text-muted-foreground">
                    {count} {count > 1 ? tr("modules") : tr("module")}
                  </span>
                </button>
              );
            })}
            {studyYears.length === 0 && (
              <p className="py-2 text-sm text-muted-foreground">{tr("Aucune année accessible.")}</p>
            )}
          </div>
        </SessionSection>
        ...
        <SessionSection
          ...
          actions={
            <button
              ...
              onClick={() =>
                setSelMods(selMods.length === yearMods.length ? [] : yearMods.map((m) => m.id))
              }
            >
              {selMods.length === yearMods.length && yearMods.length > 0
                ? tr("Tout désélectionner")
          ...
          <div className="grid max-h-56 grid-cols-1 gap-3 overflow-y-auto pr-1 md:grid-cols-2">
            {visibleMods.map((m) => (
              ...
              >
                <div className="flex min-w-0 items-center gap-3">
                  <Checkbox
                    checked={selMods.includes(m.id)}
                    onCheckedChange={() => toggle(selMods, setSelMods, m.id)}
                  />
            ...
            {visibleMods.length === 0 && (
              <p className="py-4 text-sm text-muted-foreground">
                {selYears.length === 0
                  ? tr("Choisissez d'abord une année d'étude.")
                  : tr("Aucun module accessible.")}
              </p>
            )}
          </div>
        </SessionSection>
          ...
          actions={
            <div className="flex items-center gap-2">
              <button
                ...
                className="text-xs font-medium text-primary hover:underline disabled:opacity-40"
                onClick={() => setSelFolders(folders.map((f) => f.id))}
              >
                {tr("Tout sélectionner")}
              </button>
        ...
        >
          ...
          <div className="max-h-56 space-y-2 overflow-y-auto pr-1">
            {visibleFolders.map((f) => (
              <label
                key={f.id}
                className="flex cursor-pointer items-center gap-3 rounded-xl border bg-background/60 p-3 transition hover:border-foreground/20"
              >
                <Checkbox
                  checked={selFolders.includes(f.id)}
                  onCheckedChange={() => toggle(selFolders, setSelFolders, f.id)}
                />
                <span className="text-sm font-medium">
                  {f.name}
                  <span className="ml-1 text-xs font-normal text-muted-foreground">
                    ({folderModule[f.module_id] ?? "—"})
                  </span>
                </span>
              </label>
            ))}
            {folders.length === 0 && (
              <p className="py-4 text-sm text-muted-foreground">
                {tr(
                  "Sélectionnez un module pour voir ses cours. Sans sélection, tous les cours sont inclus.",
                )}
              </p>
            )}
          </div>
        ...
        <SessionSection step={4} title={tr("Types de questions & données d'examen")}>
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
              {(Object.keys(TYPE_LABEL) as QType[]).map((ty) => (
                <TypeChip
                  ...
                  onClick={() => toggle(types, setTypes as any, ty)}
                />
              ))}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            ...
            <div>
              ...
              <Select
                value={sources[0] ?? "__all"}
                onValueChange={(v) => setSources(v === "__all" ? [] : [v])}
              >
                <SelectTrigger className="rounded-xl">
                  <SelectValue />
                </SelectTrigger>
