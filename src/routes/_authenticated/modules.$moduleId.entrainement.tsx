...
function QcmBuilder() {
  ...
  const [types, setTypes] = useState<Record<QType, boolean>>({
    qcm: true,
    qcs: true,
    qroc: true,
    cas_clinique: true,
  ...
  useEffect(() => {
    (async () => {
      const [{ data: r }, { data: f }, q, { data: mo }] = await Promise.all([
        ...
        fetchAllRows<Question>(() =>
          supabase
            .from("questions")
            .select(
              "id,type,rotation_id,folder_id,session_kind,session_number,session_title,parent_id,exam_year,exam_source",
            )
            .eq("module_id", moduleId)
            .is("parent_id", null),
        ).catch(() => [] as Question[]),
      ...
      if (u.user) {
        const { data: s } = await supabase
          .from("qcm_sessions")
          .select("id,started_at,finished_at,score,total,session_kind,session_number,mode")
          .eq("module_id", moduleId)
  ...
  const filtered = useMemo(
    () =>
      questions.filter((q) => {
        ...
        if (selFolders.length > 0 && (!q.folder_id || !selFolders.includes(q.folder_id)))
          return false;
        if (yearMin !== "__all" && (q.exam_year == null || q.exam_year < Number(yearMin)))
          return false;
        if (yearMax !== "__all" && (q.exam_year == null || q.exam_year > Number(yearMax)))
          return false;
      ...
      <main className="mx-auto max-w-4xl space-y-6 px-4 py-6 pb-32">
        ...
        >
          ...
          <Input
            ...
            placeholder={tr("Rechercher un cours…")}
            value={folderQuery}
          ...
          <div className="max-h-56 space-y-2 overflow-y-auto pr-1">
            ...
            {folders.length === 0 && (
              <p className="py-4 text-sm text-muted-foreground">
                {tr("Aucun cours pour ce module.")}
              </p>
            )}
            {folders.length > 0 && visibleFolders.length === 0 && (
              <p className="py-4 text-sm text-muted-foreground">
                {tr("Aucun cours ne correspond.")}
              </p>
            )}
          </div>
        </SessionSection>
        ...
        <SessionSection step={3} title={t("mode")}>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <ModeCard
              title={t("training")}
              description={t("immediate_correction")}
              active={mode === "training"}
              onClick={() => setMode("training")}
            />
            <ModeCard
              title={t("exam")}
              description={t("timer_end_correction")}
              active={mode === "exam"}
        ...
        >
          ...
          {sessions.map((s) => (
            <Link
              key={s.id}
              to="/modules/$moduleId/qcm/$sessionId"
              params={{ moduleId, sessionId: s.id }}
