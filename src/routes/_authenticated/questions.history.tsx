...
import { ArrowLeft, ChevronDown, ChevronRight, FolderOpen, Play, RotateCcw, Search } from "lucide-react";
...
export const Route = createFileRoute("/_authenticated/questions/history")({
  ...
  component: () => <AnyScopeGate scope="sessions"><HistoryPage /></AnyScopeGate>,
});
...
function HistoryPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data } = await supabase
        .from("qcm_sessions")
        .select("id,module_id,module_ids,title,question_ids,answers,grades,started_at,finished_at,score,total,mode,duration_seconds")
        .eq("user_id", u.user.id)
        .order("started_at", { ascending: false })
        .limit(200);
      const list = (data as Row[]) ?? [];
      setRows(list);
      const ids = Array.from(new Set(list.flatMap((r) => [r.module_id, ...(r.module_ids ?? [])]).filter(Boolean)));
      if (ids.length > 0) {
        const { data: m } = await supabase.from("modules").select("id,title,year").in("id", ids);
        const map: Record<string, string> = {};
        const ymap: Record<string, number | null> = {};
        for (const x of (m as Mod[]) ?? []) { map[x.id] = x.title; ymap[x.id] = x.year ?? null; }
        setMods(map);
        setModYear(ymap);
      }
    })();
  ...
  const groups = useMemo(() => {
    ...
    return Array.from(map.entries()).sort((a, b) => (mods[a[0]] ?? "").localeCompare(mods[b[0]] ?? ""));
  }, [rows, mods, query]);
  ...
  return (
    <div className="min-h-screen bg-background">
      <header className="border-b"><div className="mx-auto max-w-4xl px-6 py-4">
        <Button asChild variant="ghost" size="sm"><Link to="/questions"><ArrowLeft className="mr-1.5 h-4 w-4" />{t("q_hub_title")}</Link></Button>
      </div></header>
      <main className="mx-auto max-w-4xl px-6 py-10 space-y-4">
        ...
        <div className="relative">
          ...
          <Input className="pl-8" placeholder={tr("Rechercher un module ou une session…")} value={query} onChange={(e) => setQuery(e.target.value)} />
        </div>

        <Card>
          <CardHeader><CardTitle className="text-base">{rows.length} {t("sessions_count")}</CardTitle></CardHeader>
          <CardContent className="space-y-3">
            {groups.length === 0 && <p className="text-sm text-muted-foreground">{t("no_session_yet")}</p>}
            {years.map(([yearKey, entries]) => {
              ...
              return (
                <div key={yearKey} className="rounded-lg border bg-muted/30">
                  <button onClick={() => setOpenYear((p) => ({ ...p, [yearKey]: !yOpen }))}
            ...
            {entries.map(([modId, list]) => {
              ...
              return (
                <div key={modId} className="rounded-lg border">
                  <button onClick={() => setOpen((p) => ({ ...p, [modId]: !isOpen }))}
                  ...
                  {isOpen && (
                    <div className="space-y-1 border-t p-2">
                      {list.map((s) => {
                        const pct = s.total > 0 && s.score != null ? Math.round((Number(s.score) / s.total) * 100) : null;
                        ...
                        const progress = ids.length > 0 ? Math.round((answered / ids.length) * 100) : 0;
                        return (
                          <Link key={`${modId}-${s.id}`} to="/modules/$moduleId/qcm/$sessionId" params={{ moduleId: s.module_id, sessionId: s.id }}
                            className="flex items-center justify-between rounded-md border px-3 py-2 text-sm hover:bg-accent">
                            <div>
                              <div className="font-medium">{s.title ?? mods[s.module_id] ?? t("module")}</div>
                              <div className="text-xs text-muted-foreground flex flex-wrap items-center gap-1.5">
                                ...
                                {!done && <Badge className="text-[10px]">{t("in_progress")}</Badge>}
                                {multi && <Badge variant="outline" className="text-[10px]">{s.module_ids!.length} {tr("modules")}</Badge>}
                                {s.duration_seconds != null && <span>· {Math.round(s.duration_seconds / 60)} {t("min")}</span>}
                              </div>
                              {!done && ids.length > 0 && (
                                <div className="mt-1.5 sm:hidden">
                                  <Progress value={progress} className="h-1.5 w-40" />
                                  <div className="mt-1 text-[10px] text-muted-foreground">{answered}/{ids.length} {tr("répondues")}</div>
                                </div>
                              )}
                            </div>
                            <div className="flex items-center gap-3 text-right">
                              ...
                              {done && s.score != null ? (
                                <>
                                  <div className="font-semibold">{Math.round(Number(s.score) * 10) / 10}/{s.total}</div>
                                ...
                                <span className="inline-flex items-center gap-1.5 rounded-md bg-primary/10 px-2 py-1 text-xs font-medium text-primary">
                                  <Play className="h-3.5 w-3.5" />{tr("Reprendre")}
                                </span>
                              )}
                              </div>
                              {done && <RotateCcw className="h-4 w-4 text-muted-foreground" />}
                            </div>
                          </Link>
                        );
                      })}
