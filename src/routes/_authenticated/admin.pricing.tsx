...
const EMPTY_CONFIG: Config = {
  bundle_price_dzd: 0, bundle_sale_price_dzd: null, bundle_on_sale: false, bundle_free: false, currency: "DZD",
  bundle_lessons_price_dzd: 0, bundle_lessons_sale_price_dzd: null, bundle_lessons_on_sale: false, bundle_lessons_free: false,
  bundle_sessions_price_dzd: 0, bundle_sessions_sale_price_dzd: null, bundle_sessions_on_sale: false, bundle_sessions_free: false,
};


function PriceRow({ label, price, sale, on, free, onChange }: {
  label: string; price: number; sale: number | null; on: boolean; free: boolean;
    ...
    <div className="rounded-md border p-3 space-y-2">
      ...
      <div className="grid gap-3 sm:grid-cols-4">
        <div className="space-y-1"><Label>{tr("Prix")}</Label>
          <Input type="number" value={price} disabled={free} onChange={(e) => onChange({ price: Number(e.target.value) || 0 })} />
        </div>
        <div className="space-y-1"><Label>{tr("Prix promo")}</Label>
          <Input type="number" value={sale ?? ""} disabled={free} onChange={(e) => onChange({ sale: e.target.value === "" ? null : Number(e.target.value) })} />
        </div>
        ...
        <div className="space-y-1"><Label>{tr("Gratuit")}</Label>
          <div className="flex items-center gap-2 h-9">
            <Switch checked={free} onCheckedChange={(v) => onChange({ free: v })} />
            <span className="text-sm text-muted-foreground">{free ? tr("Offert") : tr("Payant")}</span>
          </div>
        </div>
      </div>
    </div>
...
function PricingAdmin() {
  ...
  const load = async () => {
    ...
    setConfig((c as unknown as Config) ?? EMPTY_CONFIG);
  };
  useEffect(() => { load(); }, []);
  ...
  const saveYear = async (yp: YearPrice) => {
    const { data, error } = await (supabase as any).from("year_prices").upsert({
      year: yp.year,
      price_dzd: yp.price_dzd, sale_price_dzd: yp.sale_price_dzd, on_sale: yp.on_sale, free: yp.free,
      lessons_price_dzd: yp.lessons_price_dzd, lessons_sale_price_dzd: yp.lessons_sale_price_dzd, lessons_on_sale: yp.lessons_on_sale, lessons_free: yp.lessons_free,
      sessions_price_dzd: yp.sessions_price_dzd, sessions_sale_price_dzd: yp.sessions_sale_price_dzd, sessions_on_sale: yp.sessions_on_sale, sessions_free: yp.sessions_free,
  ...
  const saveConfig = async () => {
    if (!config) return;
    const { data, error } = await (supabase as any).from("pricing_config").upsert({
      id: 1,
      bundle_price_dzd: config.bundle_price_dzd, bundle_sale_price_dzd: config.bundle_sale_price_dzd,
      bundle_on_sale: config.bundle_on_sale, bundle_free: config.bundle_free, currency: config.currency,
        ...
        {config && (
          <Card>
            ...
            <CardContent className="space-y-4">
              <PriceRow label={tr("Cours seuls")} price={config.bundle_lessons_price_dzd} sale={config.bundle_lessons_sale_price_dzd} on={config.bundle_lessons_on_sale} free={config.bundle_lessons_free}
              ...
              <PriceRow label={tr("Sessions seules")} price={config.bundle_sessions_price_dzd} sale={config.bundle_sessions_sale_price_dzd} on={config.bundle_sessions_on_sale} free={config.bundle_sessions_free}
                onChange={(p) => setConfig({ ...config,
              ...
              <PriceRow label={tr("Les deux")} price={config.bundle_price_dzd} sale={config.bundle_sale_price_dzd} on={config.bundle_on_sale} free={config.bundle_free}
                onChange={(p) => setConfig({ ...config,
          ...
          <CardContent className="space-y-4">
            {prices.map((yp, idx) => (
              <div key={yp.year} className="rounded-lg border p-4 space-y-3">
                ...
                <PriceRow label={tr("Cours seuls")} price={yp.lessons_price_dzd} sale={yp.lessons_sale_price_dzd} on={yp.lessons_on_sale} free={yp.lessons_free}
                  onChange={(p) => { const v = [...prices]; v[idx] = { ...yp,
                    ...
                    ...(p.free !== undefined ? { lessons_free: p.free } : {}) }; setPrices(v); }} />
                <PriceRow label={tr("Sessions seules")} price={yp.sessions_price_dzd} sale={yp.sessions_sale_price_dzd} on={yp.sessions_on_sale} free={yp.sessions_free}
                  onChange={(p) => { const v = [...prices]; v[idx] = { ...yp,
                    ...
                    ...(p.free !== undefined ? { sessions_free: p.free } : {}) }; setPrices(v); }} />
                <PriceRow label={tr("Les deux")} price={yp.price_dzd} sale={yp.sale_price_dzd} on={yp.on_sale} free={yp.free}
                  onChange={(p) => { const v = [...prices]; v[idx] = { ...yp,
                    ...(p.price !== undefined ? { price_dzd: p.price } : {}),
                    ...(p.sale !== undefined ? { sale_price_dzd: p.sale } : {}),
                    ...(p.on !== undefined ? { on_sale: p.on } : {}),
                    ...(p.free !== undefined ? { free: p.free } : {}) }; setPrices(v); }} />
