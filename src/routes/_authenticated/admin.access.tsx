...
function AccessAdmin() {
  ...
  useEffect(() => {
    if (selected) loadEnts(selected.user_id);
    else setEnts([]);
  }, [selected]);
  ...
  if (!isSuper)
    return (
      <div className="p-10 text-center text-muted-foreground">
        {tr("Réservé au super administrateur.")}
      </div>
    );
  ...
  const grant = async () => {
    ...
    if (error) toast.error(error.message);
      ...
      toast.success(tr("Accès accordé"));
      loadEnts(selected.user_id);
    }
  };

  const revoke = async (id: string) => {
    if (!(await confirm(tr("Retirer cet accès ?"), { variant: "destructive" }))) return;
    const { error } = await supabase.from("user_entitlements").delete().eq("id", id);
    if (error) toast.error(error.message);
    else {
      toast.success(tr("Retiré"));
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    <span className="font-medium truncate">
                      {u.display_name ?? tr("(sans nom)")}
                    </span>
                    {(countByUser[u.user_id] ?? 0) > 0 && (
                      <Badge variant="secondary">{countByUser[u.user_id]}</Badge>
                    )}
                  </div>
          ...
          {selected ? (
            <div className="space-y-6">
              <Card>
                <CardHeader>
                  <CardTitle className="text-base">
                    {tr("Accorder un abonnement à")}{" "}
                    {selected.display_name ?? selected.email ?? selected.user_id.slice(0, 8)}
                  </CardTitle>
                  <p className="text-xs text-muted-foreground">
                    {selected.email ?? "—"} · <span className="font-mono">{selected.user_id}</span>
                  </p>
                </CardHeader>
                <CardContent className="space-y-3">
                  <div className="grid gap-3 sm:grid-cols-4">
                    ...
                    <div className="space-y-1">
                      <Label>{tr("Formule")}</Label>
                      <Select
                        value={grantScope}
                        onValueChange={(v) => setGrantScope(v as AccessScope)}
                      >
                        <SelectTrigger>
                          <SelectValue />
                        </SelectTrigger>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base">{tr("Abonnements actuels")}</CardTitle>
                </CardHeader>
                <CardContent className="space-y-2">
                  {ents.length === 0 && (
                    <p className="text-sm text-muted-foreground">{tr("Aucun abonnement.")}</p>
                  )}
                  {ents.map((e) => (
                    <div
                      key={e.id}
                      className="flex items-center justify-between gap-2 flex-wrap rounded-md border px-3 py-2 text-sm"
                    >
                      <div className="flex items-center gap-2 flex-wrap">
                        {e.is_bundle ? (
                          <Badge>{tr("Pack complet")}</Badge>
                        ) : (
                        ...
                        {e.expires_at == null ? (
                          ...
                          <span
                            className={`text-xs ${isExpired(e.expires_at) ? "text-destructive" : "text-muted-foreground"}`}
                          >
            ...
            <Card>
              <CardContent className="py-16 text-center text-muted-foreground">
                {tr("Sélectionnez un utilisateur.")}
              </CardContent>
            </Card>
          )}
        </section>
    ...
    <Dialog open={!!row} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            {row.is_bundle ? tr("Pack complet") : `${tr("Année")} ${row.year}`} —{" "}
        ...
        <div className="space-y-4">
          ...
          {mode === "date" ? (
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap gap-2 pt-1">
                <Button
                  size="sm"
                  variant="ghost"
                  onClick={() => setValue(toLocalInput(defaultDeadline))}
                >
            ...
            <p className="text-sm text-muted-foreground">
              {tr("Cet abonnement n'expirera jamais.")}
            </p>
          )}
        </div>
        ...
        <DialogFooter>
          ...
          <Button
            disabled={busy || (mode === "date" && !value)}
            onClick={() => onSave(mode === "lifetime" ? null : new Date(value).toISOString())}
          >
