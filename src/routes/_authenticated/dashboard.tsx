...
import { FreeYear4Popup } from "@/components/FreeYear4Popup";
...
function Dashboard() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data: p } = await supabase
        .from("profiles")
        .select("display_name, avatar_url, current_year")
        .eq("id", u.user.id)
        .maybeSingle();
      setProfile(p ?? null);
      const { data: ents } = await supabase
        .from("user_entitlements")
        .select("year, is_bundle")
        .eq("user_id", u.user.id);
      const hasBundle = (ents ?? []).some((e: any) => e.is_bundle);
      const years = hasBundle
        ? [1, 2, 3, 4, 5, 6]
        : (Array.from(
            new Set((ents ?? []).map((e: any) => e.year).filter((y: any) => y != null)),
          ).sort() as number[]);
      ...
      if (finalYears.length > 0) {
        setYear(
          p?.current_year && finalYears.includes(p.current_year) ? p.current_year : finalYears[0],
        );
      }
  ...
  return (
    <div className="min-h-screen">
      <FreeYear4Popup onClaimed={() => setUnlockedYears((y) => (y.includes(4) ? y : [...y, 4]))} />
          ...
          <Card>
            <CardContent className="py-16 text-center space-y-4">
              ...
              <p className="text-muted-foreground">
                {tr("Vous n'avez encore accès à aucune année.")}
              </p>
              <Button asChild>
                <Link to="/store">
                  <ShoppingBag className="mr-1.5 h-4 w-4" />
            ...
            <div className="mb-6 flex items-end justify-between gap-4 flex-wrap">
              <div>
                ...
                <h2 className="text-3xl font-semibold tracking-tight">
                  {tr("Continuez votre préparation")}
                </h2>
                  ...
                  {isAdmin && (
                    <div className="mt-4">
                      <Button asChild size="sm">
                        <Link to="/admin">{tr("Ajouter un module")}</Link>
                      </Button>
                    </div>
                  )}
                </CardContent>
        ...
        {recentRequests.length > 0 && (
          <section className="mt-12">
            <div className="mb-3 flex items-center justify-between">
              ...
              <Button asChild variant="ghost" size="sm">
                <Link to="/store">{tr("Voir tout")}</Link>
              </Button>
            </div>
            <Card>
              <CardContent className="p-0 divide-y">
                {recentRequests.map((r) => (
                  <div
                    key={r.id}
                    className="flex items-start justify-between gap-3 px-4 py-3 text-sm flex-wrap"
                  >
                    <div className="min-w-0">
                      <div className="font-medium">
                        {r.is_bundle ? tr("Pack complet") : `${tr("Année")} ${r.year}`} —{" "}
                        {new Intl.NumberFormat("fr-FR").format(r.amount_dzd)} DZD
                      </div>
                      ...
                      <div className="mt-0.5 text-xs text-muted-foreground space-x-2">
                        {r.transaction_ref && (
                          <span>
                            {tr("Réf")} :{" "}
                            <code className="bg-muted px-1 rounded">{r.transaction_ref}</code>
                          </span>
                        )}
                        {r.proof_path && (
                          <button
