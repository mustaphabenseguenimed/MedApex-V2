...
export type Entitlement = { year: number | null; is_bundle: boolean; scope: AccessScope };

/** Does an entitlement's scope cover what's wanted? "both" covers everything. */
export function covers(entScope: AccessScope, wanted: AccessScope): boolean {
  return entScope === "both" || entScope === wanted;
}

/** Does this list of entitlements already grant `scope` access to `year` (or a bundle)? */
export function owns(
  ...
  return ents.some(
    (e) => (e.is_bundle || (!isBundle && e.year === year)) && covers(e.scope, scope),
  );
}
...
export function useModuleScope(
  ...
  scope: Exclude<AccessScope, "both">,
): ScopeStatus {
...
export function ScopeLocked({ scope }: { scope: Exclude<AccessScope, "both"> }) {
  ...
  const message =
    scope === "lessons"
      ? tr(
          "Votre formule ne comprend pas les cours. Ajoutez la formule « Cours » ou « Les deux » pour y accéder.",
        )
      : tr(
          "Votre formule ne comprend pas les sessions de questions. Ajoutez la formule « Sessions » ou « Les deux » pour y accéder.",
        );
  return (
      ...
      <Card className="max-w-md w-full">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex gap-2">
            <Button asChild>
              <Link to="/store">{tr("Voir la boutique")}</Link>
            </Button>
