  ...
  return (
    <>
      <div className="flex items-center gap-0.5">
        ...
        <button
          type="button"
          title={tr("Effacer la couleur")}
          onMouseDown={(e) => e.preventDefault()}
          onClick={() => editor.chain().focus().unsetColor().run()}
          className="ml-1 inline-flex h-5 w-5 items-center justify-center rounded hover:bg-accent text-muted-foreground"
        >
      ...
      <div className="flex items-center gap-0.5">
        ...
        <button
          type="button"
          title={tr("Retirer le surlignage")}
          onMouseDown={(e) => e.preventDefault()}
  ...
  return (
    <div className="rounded-md border bg-background">
      ...
      <div className="flex flex-wrap items-center gap-0.5 border-b px-1 py-1">
        ...
        <SwatchRow editor={editor} />
        <div className="mx-1 h-4 w-px bg-border" />
        <B onClick={() => fileRef.current?.click()} title={tr("Insérer une image")}>
          <ImagePlus className="h-3.5 w-3.5" />
        </B>
        <B onClick={runImageTest} title={tr("Tester l'aperçu des images")}>
      ...
      {placeholder && !value && (
        <div className="pointer-events-none -mt-8 px-3 pb-2 text-sm text-muted-foreground">
          {placeholder}
        </div>
      )}
      <Dialog open={previewOpen} onOpenChange={setPreviewOpen}>
        <DialogContent className="max-h-[85vh] max-w-2xl overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              {tr(
                "Rendu tel qu'il sera affiché après enregistrement (liens de stockage re-signés).",
              )}
            </DialogDescription>
          </DialogHeader>
          {imgChecks.length === 0 ? (
