...
export type ReportReason = "missing_rotation" | "wrong_answer" | "wrong_vocabulary" | "other";
...
function answerTextFor(q: CaseQuestion, tr: (s: string) => string): string {
...
export function CaseRunner({
  ...
  onReport: (q: CaseQuestion, reason: ReportReason, details?: string) => void;
  onSaveCard: (q: CaseQuestion, front: string, back: string) => void;
}) {
  const { tr } = useI18n();
  ...
  const answeredCount = useMemo(
    () => subs.filter((s) => isAnswered(s, answers)).length,
    [subs, answers],
  );
  ...
  return (
    <div className="space-y-3 pb-24">
      ...
      <Card className="overflow-hidden border-primary/40 bg-muted/40">
        <div className="flex w-full items-center gap-2 border-b bg-primary/5 px-4 py-2.5">
          <button
            type="button"
          ...
          <div className="ml-auto flex items-center gap-1">
            <Button
              ...
              onClick={() => onSaveCard(parent, parent.stem, answerTextFor(parent, tr))}
            >
            ...
            <Button
              ...
              className="h-7 px-2"
              title={tr("Flag")}
              onClick={() => onFlag(parent)}
            >
              <Flag
                className={cn("h-3.5 w-3.5", flagged.has(parent.id) && "fill-current text-primary")}
              />
            </Button>
            <Dialog>
              ...
              <DialogContent>
                <DialogHeader>
                  <DialogTitle>{tr("Signaler le cas clinique")}</DialogTitle>
                </DialogHeader>
                ...
                <DialogFooter>
                  <Button
                    onClick={() => {
                      onReport(parent, reportReason, reportDetails || undefined);
                      setReportDetails("");
                    }}
                  >
                    {tr("Envoyer")}
          ...
          <button type="button" onClick={() => setStemOpen((v) => !v)} className="shrink-0">
            <ChevronDown
              className={cn(
            ...
            >
              <AccordionTrigger className="px-4 py-3 hover:no-underline">
                <span className="flex min-w-0 flex-1 items-center gap-2.5 text-left">
                  ...
                  {flagged.has(q.id) && (
                    <Flag className="h-3.5 w-3.5 shrink-0 fill-current text-amber-500" />
                  )}
                </span>
              </AccordionTrigger>
              <AccordionContent className="space-y-3 px-4 pb-4">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {(q.type === "qcm" || q.type === "qcs") && (
                    <Badge
                      variant={q.polarity === "incorrect" ? "destructive" : "secondary"}
                      className="text-[10px]"
                    >
                  ...
                  <Button
                    ...
                    onClick={() => onSaveCard(q, q.stem, answerTextFor(q, tr))}
                  >
                    <Layers className="h-3.5 w-3.5" />
                  </Button>
                  <Button size="sm" variant="ghost" className="h-7 px-2" onClick={() => onFlag(q)}>
                    <Flag
                      className={cn(
                        "h-3.5 w-3.5",
                        flagged.has(q.id) && "fill-current text-primary",
                      )}
                    />
                  </Button>
                  <Dialog>
                    ...
                    <DialogContent>
                      ...
                      <DialogFooter>
                        <Button
                          onClick={() => {
                            onReport(q, reportReason, reportDetails || undefined);
                            setReportDetails("");
                          }}
                        >
                          {tr("Envoyer")}
                    ...
                    {grades[q.id] && (
                      <div className="rounded-lg border bg-muted/40 p-3 text-xs">
                        ...
                        {grades[q.id]?.feedback && (
                          <p className="mt-1 text-muted-foreground">{grades[q.id]?.feedback}</p>
                        )}
                      </div>
                    )}
                  </div>
